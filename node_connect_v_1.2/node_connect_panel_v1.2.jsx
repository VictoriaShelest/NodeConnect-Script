...
(function (thisObj) {
    ...
    var panel = (thisObj instanceof Panel)
        ? thisObj
        : new Window("palette", "NodeConnect", undefined, { resizeable: true });
    ...
    // Panel size
    panel.preferredSize = [260, 120];
    panel.minimumSize = [120, 40];
    ...
    var topSpacer = mainGroup.add("group");
    topSpacer.alignment = ["fill", "fill"];

    var buttonWrap = mainGroup.add("group");
    buttonWrap.orientation = "row";
    buttonWrap.alignChildren = ["center", "center"];
    buttonWrap.alignment = ["center", "center"];
    buttonWrap.minimumSize = [0, 36];
    buttonWrap.maximumSize = [600, 36];

    var btn = buttonWrap.add("button", undefined, "NodeConnect");
    btn.size = [120, 32];
    btn.minimumSize = [120, 32];
    btn.maximumSize = [120, 32];
    btn.alignment = ["center", "center"];

    var bottomSpacer = mainGroup.add("group");
    ...
    function nodeConnect() {
        ...
        if (app.project.activeItem && app.project.activeItem instanceof CompItem) {
            ...
            for (var i = 1; i <= comp.numLayers; i++) {
                var l2 = comp.layer(i);
                var m = l2.name && l2.name.match(/^Node Connection (\d+)$/);
            ...
            for (var i = 0; i < nullLayers.length - 1; i++) {
                for (var j = i + 1; j < nullLayers.length; j++) {
                    ...
                    shapeLayer.transform.anchorPoint.expression =
                        'fromComp(thisComp.layer("' + firstNullName + '").toComp(thisComp.layer("' + firstNullName + '").anchorPoint));';
                    ...
                    slider.property("Slider").expression =
                        'try{thisComp.layer("Connection CTRL").effect("NodeConnect")("Curve Amount")}catch(err){100}';
                    ...
                    stroke.property("ADBE Vector Stroke Width").expression =
                        'try{thisComp.layer("Connection CTRL").effect("NodeConnect")("Stroke Width")}catch(err){4}';
                    stroke.property("ADBE Vector Stroke Color").expression =
                        'try{thisComp.layer("Connection CTRL").effect("NodeConnect")("Stroke Color")}catch(err){[1,1,1]}';
                }
            }
