...
(function (thisObj) {
    ...
    var panel = (thisObj instanceof Panel)
        ? thisObj
        : new Window("palette", "NodeConnect 2.0", undefined, { resizeable: true });
        ...
        if (nullLayers.length < 2) {
            ...
            for (var i = 0; i < nullLayers.length - 1; i++) {
                for (var j = i + 1; j < nullLayers.length; j++) {
                    ...
                    nextNum++;
                    // Place the connection above the second null in the pair
                    shapeLayer.moveBefore(nullLayers[j]);
                    ...
                    // 1) Local curve multiplier: 0..1
                    var curveInfluence = effects.addProperty("ADBE Slider Control");
                    curveInfluence.name = "Curve Influence";
                    curveInfluence.property("Slider").setValue(1);

                    // 2) Local stroke color
                    var localColor = effects.addProperty("ADBE Color Control");
                    localColor.name = "Stroke Color Local";
                    try {
                        var globalColor = thisComp.layer("Connection CTRL").effect("NodeConnect")("Stroke Color").value;
                        localColor.property("Color").setValue(globalColor);
                    } catch (err) {
                        localColor.property("Color").setValue([1,1,1]);
                    }
                    ...
                    if (path) {
                        ...
                        expression += 'var globalCurve = 100;\n';
                        expression += 'try{ globalCurve = thisComp.layer("Connection CTRL").effect("NodeConnect")("Curve Amount"); }catch(err){}\n';
                        expression += 'var localInfluence = effect("Curve Influence")("Slider");\n';
                        expression += 'var curveCtrl = globalCurve * localInfluence;\n';
                        ...
                        expression += "  outTangents.push(i == points.length-1 ? [0,0] : [curve, 0]);\n";
                        expression += "}\n";
